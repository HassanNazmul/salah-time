...
export const filterMosquesByQuery = (mosques: Mosque[], searchQuery: string): Mosque[] => {
  ...
  return mosques.filter(
    (mosque) =>
      ...
      mosque.facilities?.some((facility: string) => facility.toLowerCase().includes(searchLower)),
  )
}
...
export const addDistancesToMosques = async (
  ...
  const mosquesWithDistances = await Promise.all(
    mosques.map(async (mosque) => {
      const distance = await calculateDistance(userPostcode, mosque.address.postcode)
      return {
        ...mosque,
        distance: distance,
      }
